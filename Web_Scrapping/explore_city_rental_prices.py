...
class SpotahomeScrapper:
    def __init__(self, url_link):
        self.url_link = url_link

    def scrape_rent_data(self):
        """
        This method scrapes the rent data from the website
        ...
        while entries_scraped < total_entries:
            url_page = self.url_link + 'page:' + str(page_num)
                    ...
                    for card in house_cards:
                        ...
                        try:
                            ...
                            available_from = card.find('span',
                                                       class_='HomecardContent_homecard-content__available-from'
                                                              '--rebranding'
                                                              '-style__X''-SyS').text
                        ...
                        writer.writerow(
                            [ad_ref, city, house_type, available_from, price_format, additional_info, ad_title,
                             link])
...
if __name__ == "__main__":
    ...
    # scrapper = SpotahomeScrapper(url)
    # scrapper.scrape_rent_data()
    scrapped_data = 'clean_data.csv'
    SpotahomeScrapper.include_property_features(scrapped_data, 'Link')
    # url = "https://www.spotahome.com/s/porto--portugal"
    # scrapper = SpotahomeScrapper(url)
    # scrapper.scrape_rent_data()
